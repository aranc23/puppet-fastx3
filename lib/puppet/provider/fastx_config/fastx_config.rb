...
# Implementation for the fastx_config type using the Resource API.
class Puppet::Provider::FastxConfig::FastxConfig < Puppet::ResourceApi::SimpleProvider
  # opens and reads the config-store.db
  ...
  def initialize
    ...
    @config_db = File.join(@fx_local_dir,'store','config-store.db')

    @configs = {}
    File.readlines(@config_db).each do |ln|
      p = JSON.parse(ln)
      if p.key?('$$deleted') and p['$$deleted'] == true
        # if this entry is marked as deleted
        if @configs.key?(p['_id'])
          @configs.delete(p['_id'])
          # delete it from the @configs hash
        end
        # skip to the next config without adding it
        next
      end
